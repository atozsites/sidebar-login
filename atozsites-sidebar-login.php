<?php
/*
Plugin Name: atozsites Sidebar Widget Login
Plugin URI: https://github.com/atozsites/sidebar-login
...
class Sidebar_Login {
	...
	public function i18n() {
		load_plugin_textdomain( 'atozsites-sidebar-login', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
	}
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'atozsites-sidebar-login', plugins_url( 'assets/js/atozsites-sidebar-login' . $suffix . '.js', __FILE__ ), array( 'jquery', 'jquery-blockui' ), $this->version, $js_in_footer );

		// Enqueue Styles
		if ( apply_filters( 'sidebar_login_include_css', true ) ) {
			wp_enqueue_style( 'atozsites-sidebar-login', plugins_url( 'assets/css/atozsites-sidebar-login.css', __FILE__ ), '', $this->version );
		}
		...
		$sidebar_login_params = array(
			...
			'is_ssl'           => is_ssl() ? 1 : 0,
			'i18n_username_required' => __( 'Please enter your username', 'atozsites-sidebar-login' ),
			'i18n_password_required' => __( 'Please enter your password', 'atozsites-sidebar-login' ),
			'error_class'      => apply_filters( 'atozsites_Sidebar_Login_Widget_error_class', 'sidebar_login_error' )
		);

		wp_localize_script( 'atozsites-sidebar-login', 'sidebar_login_params', $sidebar_login_params );
	}
	...
	public function register_widget() {
		include_once( 'includes/class-atozsites-sidebar-login-widget.php' );
	}
	...
	public function ajax_handler() {
		...
		if ( ! is_wp_error( $user ) ) {
			...
			if ( $user->errors ) {
				...
				$result['error'] = __( 'Please enter your username and password to login.', 'atozsites-sidebar-login' );
			}
		}
